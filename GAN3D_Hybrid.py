...
from tensorflow.keras.layers import Input, GlobalAveragePooling2D, Concatenate, Conv2D, Conv2DTranspose, Conv3D, Conv3DTranspose, multiply, Embedding, BatchNormalization, LeakyReLU, Flatten, Dense, Reshape, Activation, Dropout
...
class GAN3D_Hybrid():
    ...
    def build_discriminator(self):
        ...
        model.add(Conv3D(filters=8, kernel_size=(1, 1, 7), activation='relu', padding='same', input_shape=self.input_shape))
        model.add(Conv3D(filters=16, kernel_size=(3, 3, 5), activation='relu',padding='same'))
        model.add(Conv3D(filters=32, kernel_size=(5, 5, 7), activation='relu',padding='same'))
        ...
        validity = Dense(1, activation='sigmoid', kernel_initializer="he_normal")(features)
        label = Dense(self.num_classes, activation="softmax", kernel_initializer="he_normal")(features)

        return Model(img, [validity, label])
    
    def train(self, x_train, y_train, epochs, batch_size, save_interval = 50, run_name='GAN3D_HS'):  
        wandb.init(project='GAN3D_Hybrid', name=run_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='./checkpoints/GAN3D_Hybrid', help='保存模型的目录')
    ...
    for pos in range(args.repeat):
        ...
        run_name = f'GAN3D_Hybrid_{args.dataset}_b{args.batch_size}_e{args.num_epochs}_w{args.spatial_size}_n{pos}'

        if not args.test_only:
        # 训练部分
            GAN = GAN3D_Hybrid(input_shape, num_classes, latent_dim, checkpoint_dir=save_path)
            GAN.train(x_train, y_train, args.num_epochs, args.batch_size, args.save_interval, run_name = run_name)
        
        # 测试部分
        # del GAN
        GAN_D = GAN3D_Hybrid(input_shape, num_classes, latent_dim, checkpoint_dir=save_path).discriminator
